...
## v1.3.82 - 2026-03-20
- Se corrigió el tamaño del gestor de tesauros para que ocupe casi toda la pantalla útil sin forzar un modo fullscreen rígido ni alterar su distribución visual habitual.
- El modal mantiene su formato original centrado, con bordes redondeados y sombra, pero amplía su ancho y alto máximos para aprovechar mejor el espacio disponible.
...
## v1.3.81 - 2026-03-20
- El modal del gestor de tesauros ahora se abre ocupando toda la pantalla para aprovechar al máximo el área útil de edición y consulta.
- Se ajustaron los márgenes internos y la posición de los controles flotantes del gestor para mantener accesibles el cierre y el historial en formato de pantalla completa.
- Se documentó esta consolidación en el log de cambios.
...
- El backup de versión ahora incluye en el JSON el bloque `_historial.usuarioCambio` y guarda ese usuario también en el campo `user` del histórico para trazabilidad.
- En el modal de guardado, cuando ya existe un proyecto con el mismo nombre, ahora se permite elegir entre sobrescribir o guardar una copia aunque el usuario actual no sea el creador.
